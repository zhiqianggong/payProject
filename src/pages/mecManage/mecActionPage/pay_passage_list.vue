        ...
        <el-card class="box-card reset-card">
          ...
          <el-table ref="table" :data="tableData" style="width: 100%" v-loading="loading" border>
            ...
            <el-table-column prop="productType" label="产品类型" min-width="120">
              <template slot-scope="scope">{{scope.row.productType == 1 ? '收款' : '充值'}}</template>
            </el-table-column>
            ...
            <el-table-column prop="ifMode" label="接口模式" min-width="120">
              <template slot-scope="scope">
                <span
                  :style="{'color': (scope.row.ifMode  == 2 ? 'red':'')}"
                >{{scope.row.ifMode == 1 ? '单独' : '轮询'}}</span>
              </template>
            </el-table-column>
